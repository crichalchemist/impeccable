...
`context-terminal` was re-recorded earlier in this branch: the MANUAL_DETECTOR_REQUIRED directive on a terminal project now reads "Once the changed terminal UI is finished, run the mechanical detector over its source" where it previously read "Once the changed web UI is finished, run the mechanical detector over it".
