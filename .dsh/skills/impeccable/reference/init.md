...
Then configure live mode when useful: skip native, terminal, or non-runnable projects and leave existing config untouched. Otherwise follow [live.md](live.md)'s first-time setup. Any CSP source edit still requires its stated consent.
...
If init was invoked by another request, resume without rerunning `impeccable context`; the platform reference above is the one thing that run could not have given you, and new-work owns later visual decisions.
