...
This is a code-level audit, not a design critique. Audit from source (Ratatui, Bubble Tea and Lip Gloss, Textual and Rich, Ink) and from captures of the running app; no browser tooling applies. From the project root, run `.trae/skills/impeccable/scripts/impeccable detect --json <source directory>`; pass a directory, not single files, so the project signals (TTY guards, adaptive color helpers, icon fallbacks) are collected. With the app running in a tmux pane, add `--tmux <target> --tmux-sizes 80x24,120x40,40x24` to the same command for the `tui-rt-` runtime rules over the rendered grid; those are advisory too, and a saved capture replays with `--tmux-capture <file>`. Every `tui-` finding is advisory: confirm each one against the source before it enters the report. The checks below cover what detect cannot see. Score against [terminal.md](terminal.md); read it before scoring if Setup hasn't already. The report skeleton mirrors [audit.md](audit.md); keep the variants in sync when changing it.
