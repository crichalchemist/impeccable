...
- `TmuxEngine::detect_pane(target, options)`: `Tmux::locate` (`IMPECCABLE_TMUX`, else `PATH`; `tmux -V` must parse to 3.2 or newer) → `list-panes -t <target> -F '#{pane_id}'` (a missing session or window fails here with tmux's own line; tmux 3.7's `display-message -t` does not fail on a bad target) → `display-message -p -t <target> '#{pane_width}\t#{pane_height}\t#{window_width}\t#{window_height}\t#{client_termfeatures}'` → `capture-pane -p -e -J -t <target>` (frame 0, role `capture`) → sleep 1000 ms → capture again (role `recapture`) → for each `--tmux-sizes WxH`: `resize-window -t <target> -x W -y H`, sleep `--tmux-settle` (default 300), `display-message` again, capture → `resize-window` back to the recorded window size and `set-option -w -t <target> -u window-size` (always, even after a failed capture); a failed restore is reported as `window not restored: <message>`, joined to the capture error with `; ` when both fail → `rules::scan_frames`.
...
- SGR parsed: 0, 1, 2, 5/6, 7, 22, 25, 27, 30-37, 39, 40-47, 49, 90-97, 100-107, `38;5;n`, `48;5;n`, `38;2;r;g;b`, `48;2;r;g;b`, `58;5;n`, `58;2;r;g;b` (consumed, not applied); style carries across rows within a frame and resets at each header; OSC, DCS and other non-CSI escapes are skipped; a CSI with a private parameter is ignored; widths from `unicode-width` (two-cell glyphs own a wide-tail cell, zero-width marks own none).
