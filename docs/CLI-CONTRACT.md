...
- Terminal platform only: `TERMINAL_SKIP_DIRS = {'target','.venv','vendor'}` is also skipped, and `TERMINAL_EXTENSIONS = {'.rs','.go','.py','.tcss'}` is added to the scannable set. Web walks keep `SKIP_DIRS` byte-identical on every other platform.
